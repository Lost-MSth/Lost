...
inline int itrest(restrict_t& rest) {
    if (!rest.substate) {
        goto next;
    }
    {
...
inline int itrest(std::vector<restrict_t>& rest) {
...
inline uint64_t getstate(const std::vector<restrict_t>& rest) {
    uint64_t state = 0;
...
inline int generatetable(std::vector<uint64_t>& table, map_t& map,
...
template <typename VT>
inline term_t<VT> getterm(VT value, const std::vector<int>& cr,
...
template <typename VT>
inline int act(std::vector<std::vector<size_t>>& col,
               std::vector<std::vector<VT>>& data,
               const std::vector<term_t<VT>>& op,
...
inline int readss(FILE* fi, std::vector<uint64_t>& table, map_t& map) {
    int n;
    fread(&n, 1, 4, fi);
...
inline int readop(FILE* fi, std::vector<term_t<double>>& op) {
    int n, order;
    fread(&n, 1, 4, fi);
    fread(&order, 1, 4, fi);
...
inline void mmv(std::vector<double>& out, const sp_csr& m,
...
#pragma omp parallel for shared(out) schedule(static)
    for (size_t i = 0; i < row_n; i++) {
        ...
        double t = _mm512_reduce_add_pd(t_vec);
...
// v1'*v2;
inline double dot(const std::vector<double>& v1,
...
// v1+=s*v2;
inline void avv(std::vector<double>& v1, const double s,
                const std::vector<double>& v2) {
    size_t n = v1.size();
    size_t n_8 = n - n % 8;
...
// v*=s;
inline void msv(const double s, std::vector<double>& v) {
    size_t n = v.size();
    size_t n_8 = n - n % 8;
...
inline void getsp(std::vector<double>& out, int itn, const sp_csr m,
    ...
    for (int i = 0; i < itn; i++) {
        ...
        // t2 = std::chrono::steady_clock::now();
        // swap_time =
        // std::chrono::duration_cast<std::chrono::milliseconds>(swap_time + t2
        // - t1); t1 = std::chrono::steady_clock::now();
        mmv(v, m, v_);
        // t2 = std::chrono::steady_clock::now();
        // mmv_time =
        // std::chrono::duration_cast<std::chrono::milliseconds>(mmv_time + t2 -
        // t1);
        ...
        // t2 = std::chrono::steady_clock::now();
        // dot_time =
        // std::chrono::duration_cast<std::chrono::milliseconds>(dot_time + t2 -
        // t1);
        ...
        // t2 = std::chrono::steady_clock::now();
        // avv_time =
        // std::chrono::duration_cast<std::chrono::milliseconds>(avv_time + t2 -
        // t1);
    }
