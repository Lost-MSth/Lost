...
//     //      y  = y * ( 1.5 - ( x2 * y * y ) );   // 2nd iteration, this can
//     be
//     //      removed
//     return y;
// }
...
inline Vector Normalize(const Vector& v) {
    // return v * invsqrtQuake(LengthSquared(v));
    return v / Length(v);
}
...
class Camera {
   public:
    ...
    double width, height;
    Camera(Vector pos, Vector dir, Vector up, double fov,
...
bool inside_quadrilateral(double cx, double cy, Vector a, Vector b, Vector c,
                          Vector d, double& depth) {
    Vector ab = b - a, ac = c - a;

    double tmp = ab.x * ac.y - ab.y * ac.x;
    if (std::fabs(tmp) > 1e-6) {
        double inv_tmp = 1.0 / tmp;
        double cy_a = cy - a.y, cx_a = cx - a.x;
        double alpha = (-ac.x * cy_a + ac.y * cx_a) * inv_tmp;
        double beta = (ab.x * cy_a - ab.y * cx_a) * inv_tmp;
    ...
    if (std::fabs(tmp) > 1e-6) {
        double inv_tmp = 1.0 / tmp;
        double cy_d = cy - d.y, cx_d = cx - d.x;
        double alpha = (-dc.x * cy_d + dc.y * cx_d) * inv_tmp;
        double beta = (db.x * cy_d - db.y * cx_d) * inv_tmp;
        if (alpha >= 0 && beta >= 0 && alpha + beta <= 1) {
...
// rasterization
Color* render_view(const Camera& camera, const std::vector<Patch>& scene,
                   const int width, const int height) {
    Color* image = new Color[width * height];
    double* zbuffer = new double[width * height];
    double px = camera.width / width;
    double py = camera.height / height;
    double width_2 = camera.width * 0.5;
    double height_2 = camera.height * 0.5;
    bool flag = false;
    bool flag2 = false;
    ...
    for (const auto& p : scene) {
        ...
        int i = 0;
        while (i < height) {  // for (int i = 0; i < height; ++i) {
            int j = 0;
            flag = false;
            flag2 = false;

            double cy = (i + 0.5) * py - height_2;

            while (j < width) {  // for (int j = 0; j < width; ++j) {

                double cx = (j + 0.5) * px - width_2;
...
void save_bmp_file(const std::string& filename, const Color* image,
                   const int width, const int height) {
...
// divide patches into subpatches whose width and length are no less than given
// threshold
void divide_patches(std::vector<Patch>& scene, double threshold) {
    ...
    for (const auto& p : tmp) {
        ...
        for (int i = 0; i <= a; ++i) {
            for (int j = 0; j <= b; ++j) {
                Vector pos = p.pos + i * threshold * Normalize(p.a) +
                             j * threshold * Normalize(p.b);
                Vector pa = (i + 1) * threshold > len_a
                                ? p.a - i * threshold * Normalize(p.a)
                                : threshold * Normalize(p.a);
                Vector pb = (j + 1) * threshold > len_b
                                ? p.b - j * threshold * Normalize(p.b)
                                : threshold * Normalize(p.b);
                if (pa.isZero() || pb.isZero()) continue;
                scene.emplace_back(
                    Patch(pos, pa, pb, p.emission, p.reflectance));
            }
        }
    }
}
...
void cal_multiplier_map() {
    ...
    for (int i = 0; i < hemicube_res; ++i) {
        for (int j = 0; j < hemicube_res; ++j) {
            ...
            multiplier_front[i][j] =
                1 / (cx * cx + cy * cy + 1) / (cx * cx + cy * cy + 1);
    ...
    for (int i = 0; i < hemicube_res / 2; ++i) {
        for (int j = 0; j < hemicube_res; ++j) {
            ...
            double cz = (i + 0.5) * pw;
            multiplier_down[i][j] =
                cz / (cx * cx + cz * cz + 1) / (cx * cx + cz * cz + 1);
            // multiplier_down[i][j] *= multiplier_front[i + hemicube_res /
            // 2][j];
...
#pragma omp parallel for
    for (int i = 0; i < scene.size(); i++) {
        ...
        Color* front = render_view(
            Camera(cpos, Normalize(Cross(p.b, p.a)), Normalize(p.b), pi_2),
            scene, hemicube_res, hemicube_res);
        Color* up = render_view(
            Camera(cpos, Normalize(p.b), -Normalize(Cross(p.b, p.a)), pi_2),
            scene, hemicube_res, hemicube_res);
        Color* left =
            render_view(Camera(cpos, -Normalize(p.a), Normalize(p.b), pi_2),
                        scene, hemicube_res, hemicube_res);
        Color* right =
            render_view(Camera(cpos, Normalize(p.a), Normalize(p.b), pi_2),
                        scene, hemicube_res, hemicube_res);
        Color* down = render_view(
            Camera(cpos, -Normalize(p.b), Normalize(Cross(p.b, p.a)), pi_2),
            scene, hemicube_res, hemicube_res);
        Color total_light{};
        ...
        for (int i = 0; i < hemicube_res; ++i) {
            for (int j = 0; j < hemicube_res; ++j) {
                total_light = total_light + front[i * hemicube_res + j] *
                                                multiplier_front[i][j];
                if (i < hemicube_res / 2) {
                    total_light =
                        total_light +
                        up[i * hemicube_res + j] *
                            multiplier_down[hemicube_res / 2 - 1 - i][j];
                } else {
                    total_light = total_light +
                                  down[i * hemicube_res + j] *
                                      multiplier_down[i - hemicube_res / 2][j];
                }
                if (j < hemicube_res / 2)
                    total_light =
                        total_light +
                        right[i * hemicube_res + j] *
                            multiplier_down[hemicube_res / 2 - 1 - j][i];
                else {
                    total_light = total_light +
                                  left[i * hemicube_res + j] *
                                      multiplier_down[j - hemicube_res / 2][i];
                }
            }
        }
...
int main() {
    ...
    Camera camera(Vector(278, 273, -800), Vector(0, 0, 1), Vector(0, 1, 0),
                  2 * std::atan2(0.0125, 0.035));
    ...
    save_bmp_file("cornellbox" + std::to_string(iter) + ".bmp", image, width,
                  height);
    delete[] image;
    ...
    for (iter = 1; iter <= max_iteration; ++iter) {
        ...
        std::cout << "save image " << iter << std::endl;
        save_bmp_file("cornellbox" + std::to_string(iter) + ".bmp", image,
                      width, height);
        delete[] image;

        tt2 = std::chrono::steady_clock::now();
        std::cout << "time: "
                  << std::chrono::duration_cast<std::chrono::milliseconds>(tt2 -
                                                                           tt1)
                         .count()
                  << "ms" << std::endl;
    }
