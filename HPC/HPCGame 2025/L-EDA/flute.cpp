...
#if PRINT
std::vector<double> duration_1(THREAD_NUM, 0);
std::vector<double> duration_2(THREAD_NUM, 0);
std::vector<int> count_1(THREAD_NUM, 0);
std::vector<int> count_2(THREAD_NUM, 0);

std::vector<double> duration_3(THREAD_NUM, 0);
std::vector<int> count_3(THREAD_NUM, 0);
#endif
...
#pragma omp parallel for num_threads(THREAD_NUM) schedule(static)
    ...
    for (int i = 0; i < THREAD_NUM; ++i) {
        printf("Duration 1 of %d: %f  Count: %d\n", i, duration_1[i] / 1e9,
               count_1[i]);
        printf("Duration 2 of %d: %f  Count: %d\n", i, duration_2[i] / 1e9,
               count_2[i]);
        // printf("Duration 3 of %d: %f  Count: %d\n", i, duration_3[i] / 1e9,
        //        count_3[i]);
    }
#endif
