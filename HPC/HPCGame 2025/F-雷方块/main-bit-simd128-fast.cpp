...
inline void bit_add(ARR &a1, ARR &a2, const ARR &b1, const ARR &b2) {
	...
	for (int i = 0; i < MAX_N / 128; i++) {
		...
		__m128i a1_result = _mm_or_si128(_mm_andnot_si128(a2_b2, nonzero), _mm_andnot_si128(nonzero, _mm_xor_si128(a1_val, b1_val)));
		__m128i a2_result = _mm_or_si128(_mm_andnot_si128(_mm_or_si128(a2_val, b2_val), nonzero), _mm_andnot_si128(nonzero, a2_b2));
...
void solve(SparseMatrixRow &A, std::vector<u8> &B, std::vector<u8> &X, int n1) {
    ...
    for (int i = 0; i < n; i++) {
		...
		// 		printf("%3d", get_ele(rows_first[ii], rows_second[ii], jj - nnz_idx[ii]));
        // 	}
