...
assert len(REGIONS) == HEIGHT
assert all(len(row) == WIDTH for row in REGIONS)
...
def one_try(REGION_COUNTS):
    """Heyawake solver example."""
    sym = grilops.SymbolSet([("B", 'x'), ("W", 'o')])
    ...
    # 优化，左上角两个格子不可能全黑，有一个白格可以作为 root
    sg.solver.add(Or(
        white_region_id == 0,
        white_region_id == 1,
    ))
    ...
    for region, count in REGION_COUNTS.items():
        sg.solver.add(PbEq([(c == sym.B, 1)
                      for c in region_cells[region]], count))
...
def main():
    # for i in range((WIDTH * HEIGHT - 4*6)//2, -1, -1):
    #     print(f'Trying REGION_COUNTS = {{"A": {i}}}')
    #     one_try({"A": i})
    #     print()
