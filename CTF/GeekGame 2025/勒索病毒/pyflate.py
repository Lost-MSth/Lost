    ...
    def find_next_symbol(self, field, reversed=True):
        ...
        for x in self.table:
            ...
            if (reversed and x.reverse_symbol == cached) or (not reversed and x.symbol == cached):
                ...
                print("found symbol", bin(cached), chr(cached), hex(cached), "of len",
                      cached_length, "mapping to", x.code, hex(x.code))
                return x.code
...
class OrderedHuffmanTable(HuffmanTable):
    def __init__(self, lengths):
        l = len(lengths)
        z = list(zip_longest(list(range(l)), lengths)) + [(l, -1)]
...
def bzip2_main(input):
    ...
    while True:
        ...
        if blocktype == 0x314159265359:  # (pi)
            ...
            favourites = [y for x, y in map(None, used, list(
                map(chr, list(range(len(used)))))) if x]
...
def main_for_deflate(input):
    ...
    while True:
        ...
        if blocktype == 0:
            ...
            for x in main_literals.table:
                print('symbol', x.code, 'length',
                      x.bits, 'code', bin(x.symbol))
                if x.code < 256:
                    print('literal', repr(chr(x.code)))
                if 257 <= x.code <= 285:
                    print('length base', length_base(x.code),
                          'extra bits', extra_length_bits(x.code))

            while True:
                lz_start = b.tellbits()
                print('LZ at', b.tell(), 'bits', b.tellbits() - bheader_start)
                r = main_literals.find_next_symbol(b)
                if 0 <= r <= 255:
                    if literal_count == 0:
                        literal_start = lz_start
                    ...
                    length_extra = b.readbits(extra_length_bits(r))
                    length = length_base(r) + length_extra
                    print('length', length, 'len extra', length_extra)
                    ...
                    if 0 <= r1 <= 29:
                        ...
                        distance = distance_base(
                            r1) + b.readbits(extra_distance_bits(r1))
                        print('distance_base', distance_base(r1), 'extra', extra_distance_bits(r1))
                        print('distance', distance)  # 永远是 1，因为 r1 永远是 0
                        cached_length = length
                        while length > distance:
                            out += out[-distance:]
                            length -= distance
