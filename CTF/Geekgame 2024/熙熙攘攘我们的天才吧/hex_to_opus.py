...
__all__ = ['ogg_opus_coder', 'srtp_ogg_opus_coder', 'ogg_page_coder',
           'RTP_HEADER', 'RTP_HEADER_FMT', 'RTP_FIXED_HEADER']
__author__ = "Kamanashis Roy"
__copyright__ = "Copyright (C) 2019 Kamanashis Roy"
__license__ = "GNU Public License version 3"
__version__ = "1.0"

OGG_PAGE_HEADER = namedtuple(
    "OGG_PAGE_HEADER", "PATTERN,VERSION,HEADER_TYPE,GRANULE_POS,BITSTREAM,PAGE_SEQ,CRC,NUM_SEGMENTS")
...
class ogg_page_coder:
    ...
    def _make_page_header(self, content_len, ptime, is_data, is_first, is_last, crc):
        ...
        if content_len & 0xFF:
            num_segments += 1
        header = OGG_PAGE_HEADER(b'OggS', 0, header_type_flag, granule_position,
                                 self.curr_bitstream, self.page_seq, crc, num_segments)
        ...
        assert len(page_head) == (27+num_segments)
    ...
    def write_page(self, content, ptime=20, is_data=False, is_first=False, is_last=False, pageno=0):
        ...
        head = self._make_page_header(content_len=len(
            content), ptime=ptime, is_data=is_data, is_first=is_first, is_last=is_last, crc=0)
        ...
        # put correct crc
        head = self._make_page_header(content_len=len(
            content), ptime=ptime, is_data=is_data, is_first=is_first, is_last=is_last, crc=crc)
    ...
    def __next__(self):
        ...
        page_head = OGG_PAGE_HEADER._make(
            struct.unpack(OGG_PAGE_HEADER_FMT, page_head_raw))
...
OPUS_IDENTITY_HEADER = namedtuple(
    "OPUS_IDENTITY_HEADER", "PATTERN,VERSION,NUM_CHANNELS,PRE_SKIP,SAMPLING_RATE,GAIN,FAMILY")
OPUS_IDENTITY_HEADER_FMT = '<8sBBHIhB'

RTP_HEADER = namedtuple(
    'RTP_HEADER', 'FIRST,PAYLOAD_TYPE,SEQUENCE_NUMBER,TIMESTAMP,SSRC')
...
class ogg_opus_coder:
    ...
    def write_stream_header(self, bitstream_serial):
        ...
        # version 1, channel 1, pre-skip, freq 48kHz, gain 8, mono = 0
        header = OPUS_IDENTITY_HEADER(
            b'OpusHead', 1, 1, self.pre_skip, self.sampling_rate, self.gain, 0)
...
class srtp_ogg_opus_coder(ogg_opus_coder):
    ...
    def record_rtp_packet(self, rtp_payload, no, is_last=False):
        assert self.ogg is not None

        self.ogg.write_page(rtp_payload, is_data=True, is_last=is_last,
                            is_first=(no == 0),
                            ptime=20, pageno=no)  # By default the ptime=20
        return True
    ...
    def record_rtp_file(self, infile, outfile, hexstring=False):
        ...
        if hexstring:
            with open(infile, 'r') as rtp_fd:
                self.write_stream_header(0)
                self.write_stream_comment('hello')
                packet_counter = 0
                success_counter = 0

                for xline in rtp_fd:
                    xline = xline.strip()
                    no, packet = xline.split(',')
                    packet_counter += 1
                    if self.record_rtp_packet(bytes.fromhex(packet), int(no), is_last=int(no) == 2117):
                        success_counter += 1
        else:
            ...
            for header, content in self.ogg:
                ...
                #    print(opus_comment_head)
                    ...
                    if page_counter > 1:
                        ...
                        # make an RTP packet
                        rtp = RTP_HEADER(
                            0x80, self.payload_type, sequence_counter, header.GRANULE_POS, header.BITSTREAM)
                        rtp_raw_full = struct.pack(
                            RTP_HEADER_FMT, *rtp) + content
                        ...
                        if self.srtpkey:
                            if self.ssrc != rtp.SSRC:
                                ...
                                plc = Policy(
                                    key=srtpkey, ssrc_value=rtp.SSRC, ssrc_type=Policy.SSRC_ANY_OUTBOUND)
                                print(plc)
                            ...
                            try:
                                ...
                                print("encrypt fail seq={sequence}, ssrc={ssrc}".format(
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description='RTP hex-dump reader/writer tool for OPUS payload')
    ...
    explain_group.add_argument(
        '--explainfile', required=False, help='ogg formatted opus input file')

    record_group = parser.add_argument_group('Record rtp/srtp stream')
    record_group.add_argument(
        '-r', '--recordfile', required=False, help='ogg formatted opus output file')

    record_group.add_argument('--payloadoffset', required=False, type=int, default=None,
    ...
    record_group.add_argument(
        '--rtpoffset', required=False, type=int, default=0, help='Offset of rtp header')

    record_group.add_argument('--ssrc', required=False, type=int, default=None,
                              help='Specify ssrc as it appears in SDP. It is used to filter-out specific stream')

    playfile_group = parser.add_argument_group(
        'Playback opus file into hex-stream')
    playfile_group.add_argument('--playfile', required=False,
                                help='ogg formatted opus input file that is streamed into rtp hex file')

    playfile_group.add_argument('--streamseq', required=False, type=int, default=0,
                                help='Specify starting sequence while creating playback rtp stream')

    # Support encryption/decryption
    parser.add_argument('-k', '--srtpkey', required=False,
                        default=None, help='srtpkey to encrypt/decrypt the rtp')

    parser.add_argument('--resrtpkey', required=False, default=None, help='srtp re-key/second-key to decrypt the rtp\n'
                        'It is used when the srtpkey fails to decrypt valid rtp-payload.')

    parser.add_argument('--payloadtype', required=False, type=int, default=111,
                        help='Specify payload type as it appears in SDP. It is used to filter-out RTCP packets')

    parser.add_argument('-x', '--hexfile', required=False, help='Text output of pcap, hint `tshark -x -r file.pcap -Y "rtp && udp.srcport == myport" | cut -d " " -f 1-20 > strm_from_myport.txt`\n'
                        'In case of recording, it is input file. In case of playback, it is the output file')

    parser.add_argument('--hexstring', action='store_true', help='Text output of pcap, hint `tshark -x -r file.pcap -Y "rtp && udp.srcport == myport" -T fields -e data > strm_from_myport.txt`\n'
                        'In case of recording, it is input file. Playback is not supported in this format.')

    args = parser.parse_args()

    args.hexfile = 'audio.txt'
    args.recordfile = 'audio.opus'
    args.hexstring = True

    if args.verbose:
        print(args)
    if args.hexfile and args.recordfile:
        opusfile = srtp_ogg_opus_coder()
        # opusfile = srtp_ogg_opus_coder(override_payload_offset=args.payloadoffset, rtp_offset=args.rtpoffset, verbose=args.verbose,
        #    srtpkey=args.srtpkey, resrtpkey=args.resrtpkey, payload_type=args.payloadtype, filter_ssrc=args.ssrc)
        opusfile.record_rtp_file(args.hexfile, args.recordfile, args.hexstring)
    if args.explainfile:
        opusfile = ogg_opus_coder(verbose=args.verbose)
        opusfile.explain_file(args.explainfile)

    if args.playfile:
        opusfile = srtp_ogg_opus_coder(override_payload_offset=args.payloadoffset, rtp_offset=args.rtpoffset,
                                       verbose=args.verbose, srtpkey=args.srtpkey, payload_type=args.payloadtype, filter_ssrc=args.ssrc)
        opusfile.playback_as_srtp_stream(
            args.playfile, args.hexfile, args.streamseq)
